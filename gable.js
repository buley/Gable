		...
		for (var attr in value) {
			...
			if (Gable.utils.isArray(value[attr])) {
				...
				if( null === biggest || ( true === nonulls && valuelen >= biggest_total && false === haveId ) ) {
					biggest = value[attr];
					if( 'undefined' !== typeof value[attr].id || 'object' === typeof value[attr] && !Gable.utils.isArray( value[attr] ) ) {
						haveId = true;
					}
			...
			if(  'object' === typeof val ) {
				if( Gable.utils.isArray(val) ) {
					...
					for( var y = 0; y < val.length; y += 1 ) {
						if( Gable.utils.isArray(val[y]) ) {
							for( var z = 0; z < val[y].length; z += 1 ) {
								var rw;	
								if( 'object' === typeof val[ y ][ z] && !Gable.utils.isArray( val[y][ z ] ) && !( val[y][ z ] instanceof Date ) ) {
									tmpstack = [];
