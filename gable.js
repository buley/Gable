var Gable = (function () {
    ...
    Public.prototype.dump = function () {
        ...
        if (null === req) {
            ...
            Public.prototype.get({
                'row': (req.row - 1),
                'column': (req.column - 1),
                'on_success': dump_on_success,
                'on_error': dump_on_error
            });
        }
    ...
    Public.prototype.find = function () {
        ...
        var find_id = arguments[0],
            find_ids, table_id = current_table,
            find_result, types = null;
        var Find = function () {
        ...
        Find.prototype.dump = function () {
            ...
            var find_item, findlen = find_result.length,
                x = 0;
    ...
    Private.data.types.raw.transform.table = function (obj) {
        ...
        for (x = 0; x < colcount; x += 1) {
            ...
            if (null === item || 'undefined' === typeof item) {
                ...
                addition['p'] = null;
                addition['type'] = null;
            } else if ('undefined' !== typeof item.id && null !== item.id) {
                addition['id'] = item.id;
    ...
    Private.data.types.raw.transform.csv = function (obj, options) {
        ...
        if ('undefined' !== typeof obj) {
            ...
            if ('undefined' !== typeof obj.type) {
                ...
                on_error({
                    'message': 'Not a recognized CSV data structure.'
                });
            ...
            if ('column' === type) {
                ...
                if ('undefined' !== typeof table.rows) {
                    ...
                    for (y = 0; y < rowlen; y += 1) {
                        ...
                        if (null !== newval && 'undefined' === typeof newval) {
                            newval = null;
                        }
                        var id, val;
                        if (true === use_id) { //should be behavior using id?
                            if (null !== table.rows[y].id && 'undefined' !== typeof table.rows[y].id) {
                                id = table.rows[y].id;
                            } else {
                                id = null;
                ...
                for (x = 0; x < len; x += 1) {
                    ...
                    var rwitems = obj.rows[x].value,
                        a = 0,
                        rwlen = rwitems.length,
                        rwitem;
                    for (a = 0; a < rwlen; a += 1) {
